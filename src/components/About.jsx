...
export default function About() {
    ...
    const [images, setImages] = useState({
        main: 'https://images.unsplash.com/photo-1521791136064-7986c2920216?w=700&q=80',
        secondary: 'https://images.unsplash.com/photo-1554224155-6726b3ff858f?w=400&q=80',
    });
    ...
    useEffect(() => {
        api.getContentBySection('images').then((data) => {
            if (data) {
                setImages(prev => ({
                    main: data.about_image_1 || prev.main,
                    secondary: data.about_image_2 || prev.secondary,
                }));
            }
        }).catch(() => { });
    }, []);
                ...
                <div ref={ref} className="grid lg:grid-cols-2 gap-16 items-center mb-12 lg:mb-24">
                    ...
                    <div className={`animate-fade-up ${isVisible ? 'visible' : ''}`}>
                        ...
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <span className="text-sm font-medium text-emerald-brand tracking-wider uppercase">{t('about.whyChoose')}</span>
                        </div>
                    ...
                    <div className={`relative animate-fade-up stagger-2 ${isVisible ? 'visible' : ''}`}>
                        <div className="relative">
                            <img
                                src={images.main.startsWith('/uploads') ? (import.meta.env.VITE_API_URL ? import.meta.env.VITE_API_URL.replace('/api', '') + images.main : images.main) : images.main}
                                alt="People shaking hands, symbolizing partnership"
                                className="w-full h-80 object-cover rounded-xl shadow-lg"
                            />
                            {/* Overlapping smaller image */}
                            <img
                                src={images.secondary.startsWith('/uploads') ? (import.meta.env.VITE_API_URL ? import.meta.env.VITE_API_URL.replace('/api', '') + images.secondary : images.secondary) : images.secondary}
